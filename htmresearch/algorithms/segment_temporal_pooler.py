...
class UnionTemporalPooler(SpatialPooler):
  """
  Experimental segment-based Temporal Pooler Python implementation. The segment
  Temporal Pooler is based on the Union Temporal Pooler, which builds a
  ...
  Unlike the Union Temporal Pooler, these predictive-active inputs will tend to
  be grouped together onto segments (as in the temporal memory).  These segments
  provide a nonlinear increase in their voting power, allowing the Segment
  Temporal Pooler to more easily remember important combinations of cells.
  """
  ...
  def __init__(self,
               ...
               segmentFunction = lambda x: x + x**2,
               decayTimeConst=20.0,
    ...
    self._learningThreshold = learningThreshold
    ...
    # Segments of cells in the spatial pooler
    self._cellSegments = [[]]*self.getNumColumns()

    # Function used to increase weight of input on segments
    self._segmentFunction = segmentFunction

    # lowest possible pooling activation level
    self._poolingActivationlowerBound = 0.1

    # Previous active input
  ...
  def calculateSegmentOverlaps(self, input, predictedActiveInput):
    activations = numpy.zeros(self.getNumColumns(), dtype=REAL_DTYPE)
    for cell, segments in enumerate(self._cellSegments):
      for segment in segments:
        segment_activation = numpy.sum([1 for i in segment if input[i] == 1])
        segment_activation += self.predictedActiveOverlapWeight*numpy.sum(
            [1 for i in segment if predictedActiveInput[i] == 1])
        activations[cell] += self._segmentFunction(segment_activation)
  ...
  def compute(self, activeInput, predictedActiveInput, learn):
    ...
    segmentOverlaps = self.calculateSegmentOverlaps(activeInput, predictedActiveInput)
    totalOverlap = (overlapsActive * self._activeOverlapWeight +
                    overlapsPredictedActive *
                    self._predictedActiveOverlapWeight +
                    segmentOverlaps).astype(REAL_DTYPE)
  ...
  def _adaptSegments(self, predictedActiveInput, activeColumns):
    """
    Updates the segments in the input.  Will modify the permanences of inputs
    on segments based on their activation patterns, using the same logic that
    is employed in the TM.  Predicted active inputs can be added to a segment
    that they are correlated with (i.e. a segment with activity >
    learningThreshold).

    Currently awaiting full implementation.
    """
    raise NotImplementedError
    for cell in activeColumns:
      activeSegments = []
      for i, segment in enumerate(self._cellSegments[cell]):
        activation = numpy.sum([1 for i in segment if predictedActiveInput[i] == 1])
        if activation > self._learningThreshold:
          activeSegments.append(i)


  # override
  def _adaptSynapses(self, inputVector, predictedActiveInput, activeColumns, synPermActiveInc, synPermInactiveDec):
    """
